...
- 정성욱 (대표이사) — 일손 단독 운영 (PM + 개발 + 머지 권한 전부). staff_id 미정 (일손 별도 staff 테이블)

### 향후 역할 부여 가능 (현재 일손 미관여, 역할 미정)
- 신규 PM (합류 예정) — 일손 인계 대상
- 김주은 (쥬니) — 현재 다웰솔루션 전담
- 이지수 — 개발 관련 역할 가능성
...
조사 PR은 외부 결정(향후 PM·운영 등) 답변 대기 시에만 분리.
...
7. PR 머지: 현재 일손 단독 운영자(정성욱)가 GitHub에서 직접 머지. 본인 검토(`/review`)를 거친 뒤 머지하며, 셀프 머지여도 PR 단위 이력은 유지(직접 main push 지양). 추후 PM 합류 시 머지 승인자 재지정.
